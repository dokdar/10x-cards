...
2. Each flashcard must have:
   ...
   - source_fragment: The exact text fragment from source that supports this flashcard (max 300 characters)
...
[
  {
    ...
    "source_fragment": "Relevant excerpt from source text"
  }
]
  ...
  private parseAIResponse(response: OpenRouterResponse): FlashcardCandidate[] {
    ...
    try {
      ...
      for (const candidate of candidates) {
        if (!candidate.front || !candidate.back || !candidate.source_fragment) {
