...
vi.mock('../CandidateCard', () => ({
  CandidateCard: ({ candidate, onUpdate, onToggleAccept, onReject }: {
    ...
    <div data-testid={`candidate-card-${candidate.id}`}>
      ...
      <button
        onClick={() => onToggleAccept(candidate.id)}
      ...
      <button
        onClick={() => onUpdate(candidate.id, 'front', 'Updated front')}
        data-testid={`update-${candidate.id}`}
      >
        Update
      </button>
    </div>
